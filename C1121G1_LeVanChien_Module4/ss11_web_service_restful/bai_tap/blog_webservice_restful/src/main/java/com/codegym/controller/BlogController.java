...
public class BlogController {
    ...
    @GetMapping({"", "/list"})
    public ResponseEntity<Page<Blog>> goBlogList(@PageableDefault(value = 3) Pageable pageable, @RequestParam Optional<String> search) {
    ...
    @GetMapping("/search")
    public ResponseEntity<List<Blog>> findByCategoryName(@RequestParam String categoryName) {
        List<Blog> blogs = iBlogService.findByCategory(categoryName);
        return new ResponseEntity<>(blogs, HttpStatus.OK);
    }
